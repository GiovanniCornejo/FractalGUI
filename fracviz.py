...
class FractalWindow(QWidget):
    ...
    def __init__(self, file: str, app: FractalApp):
        ...
        self.layout = self.get_child(window, QVBoxLayout, "layout")
        self.iterations = self.get_child(window, QLineEdit, "iterations")
        self.processes = self.get_child(window, QLineEdit, "processes")
        self.resolution_x = self.get_child(window, QLineEdit, "resolution_x")
        self.resolution_y = self.get_child(window, QLineEdit, "resolution_y")
        self.reset_button = self.get_child(window, QPushButton, "reset_button")
        self.status = self.get_child(window, QLabel, "status")
